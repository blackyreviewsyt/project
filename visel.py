...
def but():
    ...
    while y < 600:
        x = 0
        while x < 600:
            canvas.create_rectangle(x, y, x+33, y+27, fill="white", outline="black")
            x = x+33
        y = y+27
...
fag = '''привет! Давай поиграем!
Игрокам загадывается слово,
которое они должны угадать,
используя выданные буквы алфавита
и возможность совершить ограниченное количество ошибок,
совершить можно только 6 ошибок.
'''
canvas.create_text(310, 240, text=fag, fill="purple", font=("Helvetica", "14"))
slova = ["виселица", "смартфон", "безумный", "веснушка", "гвоздика", "страница", "микрофон"]
...
def arr():
    ...
    def end():
        canvas.create_text(180,150, text="Ты проиграл!!", fill="purple", font=("Helvetica", "18"))
        for i in alfabet:
            btn[i]["state"] = "disabled"
